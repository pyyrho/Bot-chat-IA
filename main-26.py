...
class RevolutxBot(commands.Bot):
    ...
    async def _rotate_status(self) -> None:
        ...
        # discord.py 2.x aceita lista de activities:
        # CustomActivity = status em cima (com emoji de texto)
        # Streaming      = atividade embaixo (ícone roxo "Transmitindo")
        await self.change_presence(
            status=discord.Status.online,
            activities=[
                discord.CustomActivity(name=custom_text),
                discord.Streaming(name=_STREAMING_NAME, url=_STREAMING_URL),
            ],
        )
