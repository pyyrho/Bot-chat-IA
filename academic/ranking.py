...
_SOURCE_WEIGHTS = {
    ...
    "arXiv": 1.05,
    "Crossref": 1.0,
    "Springer Nature via Crossref": 1.25,
}
...
def merge_works(works: list[AcademicWork]) -> list[AcademicWork]:
    ...
    for work in works:
        ...
        if existing:
            ...
            # Um registro mais completo pode acrescentar DOI/identificadores ao
            # primeiro. Atualiza os índices para que os próximos resultados ainda
            # sejam agrupados, mesmo quando vierem com outro título.
            if existing.doi:
                doi_index[existing.doi.lower()] = existing
            for key, value in existing.identifiers.items():
                if value:
                    id_index[f"{key}:{value.lower()}"] = existing
        else:
            merged.append(work)
            if work.doi:
    ...
    *,
    ...
    return sorted(
        ...
        key=lambda w: (
            w.is_retracted,
            -w.score,
            -(w.citation_count or 0),
            -(w.year or 0) if prefer_recent else 0,
        ),
    )[:limit]
