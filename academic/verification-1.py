...
class ReferenceAudit:
    ...
    def compact(self) -> str:
        ...
        for check in self.checks[:12]:
            flags = []
            if not check.has_persistent_id:
                flags.append("identificador persistente ausente")
            elif not check.valid_doi_shape:
                flags.append("sem DOI; identificado por outra base")
...
def audit_references(works: list[AcademicWork]) -> ReferenceAudit:
    ...
    for index, work in enumerate(works, 1):
        ...
        valid_doi = bool(doi and _DOI_RE.match(doi))
        persistent_ids = {
            key: value
            for key, value in work.identifiers.items()
            if key.lower() in {"pmid", "pmcid", "arxiv", "openalex", "s2"} and value
        }
        checks.append(
            ReferenceCheck(
                source_index=index,
                valid_doi_shape=valid_doi,
                has_persistent_id=valid_doi or bool(persistent_ids),
