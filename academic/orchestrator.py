...
class AcademicOrchestrator:
    ...
    async def _search_uncached(self, question: str, analysis: QueryAnalysis) -> AcademicSearchResult:
        ...
        per_connector = 6 if plan.depth in {"deep", "research"} else 4
        ...
        for connector in self.connectors:
            ...
            elif connector.name == "arXiv":
                technical_query = next(
                    (
                        q for q in plan.queries
                        if any(
                            marker in q.lower()
                            for marker in ("approaches", "methods", "theorem", "proof", "survey", "benchmark")
                        )
                    ),
                    None,
                )
                query = technical_query or query
        ...
        ranked = rank_works(
            ...
            prefer_recent=plan.recent_only,
        )
        ...
        sources = []
        for index, work in enumerate(ranked, start=1):
            link = work.url or (f"https://doi.org/{work.doi}" if work.doi else "sem link público")
            sources.append(f"[S{index}] {work.title} | {work.source} | {link}")
