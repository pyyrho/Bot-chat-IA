...
class AIChat(commands.Cog):
    def __init__(self, bot):
        ...
        # Groq — suporte a múltiplas chaves (rotação automática)
        self.groq_keys = []
        for i, var in enumerate(["GROQ_API_KEY", "GROQ_API_KEY_2", "GROQ_API_KEY_3", "GROQ_API_KEY_4", "GROQ_API_KEY_5"], 1):
            key = os.getenv(var)
            if key:
                self.groq_keys.append(Groq(api_key=key))
                logger.info(f"✅ {var} carregada ({key[:8]}...)")
        if not self.groq_keys:
            logger.error("❌ Nenhuma GROQ_API_KEY encontrada!")
        else:
            logger.info(f"✅ {len(self.groq_keys)} chave(s) Groq carregada(s)")
        self.groq_index = 0  # índice da chave atual
    ...
    async def get_ai_response(self, user_message: str, user_id: int, user_name: str) -> str:
        ...
        # Tokens generosos para nunca cortar respostas
        max_tokens = 1500 if academic else 900
        temperature = 0.4 if academic else 0.95

        # ── Tenta todas as chaves Groq em sequência ──────────────
        messages = [
            {"role": "system", "content": get_system_prompt() + anti_repeat}
        ] + history

        for attempt in range(len(self.groq_keys)):
            client = self.groq_keys[self.groq_index % len(self.groq_keys)]
            key_num = (self.groq_index % len(self.groq_keys)) + 1
            try:
                ...
                if is_auth:
                    logger.error(f"Chave Groq #{key_num} inválida")
                elif is_limit:
                    logger.warning(f"Chave Groq #{key_num} com limite — tentando próxima")
                else:
                    logger.error(f"Erro Groq #{key_num}: {type(e).__name__}: {e}")

                # Avança pra próxima chave
                self.groq_index += 1
    ...
    async def parse_mod_command(self, message: str, mentions: list) -> dict:
        ...
        try:
            client = self.groq_keys[self.groq_index % len(self.groq_keys)] if self.groq_keys else None
            if not client:
                return {"action": "none"}
            response = await asyncio.to_thread(
                client.chat.completions.create,
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Se for uma reply, inclui o contexto da mensagem original
        if is_reply_to_bot and message.reference and message.reference.resolved:
            ...
            if original_content and len(original_content) > 10:
                # Injeta a mensagem anterior como contexto para o modelo continuar
                truncated = original_content[:500] + ("..." if len(original_content) > 500 else "")
                context_hint = f"[Contexto — sua resposta anterior foi: '{truncated}'] Agora o usuário diz: {content}"
        ...
        try:
            if len(response) > 1900:
                # Divide por parágrafos para não cortar no meio de uma frase
