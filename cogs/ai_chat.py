...
AI_CHAT_BUILD = "2026-08-21-academico-v3-fonte-primeiro-save-only-gemini-3.6"
...
class AIActionView(discord.ui.View):
    """Controle mínimo pós-resposta: somente salva a resposta da sessão V3."""
    ...
    async def save_button(
        ...
        try:
            status = await asyncio.wait_for(
                self.cog._save_academic_response_to_project(
                    interaction,
            ...
            status = (
                "O salvamento demorou mais que o esperado e foi interrompido. "
...
class AIChat(commands.Cog):
    ...
    async def _gemini_generate_once(
        ...
        *,
        ...
        model_name = GEMINI_MODEL.lower().strip()
        config_kwargs: dict[str, Any] = {
            "system_instruction": system_prompt or None,
            "max_output_tokens": max(32, int(max_tokens)),
        }
        # Gemini 3.x foi calibrado para usar os valores de amostragem padrão.
        # Manter temperature/top_p ausentes evita loops e perda de qualidade em
        # matemática e raciocínio. Modelos anteriores preservam o ajuste legado.
    ...
    def _make_view(
        ...
        *,
        ...
        if (
            disable_buttons
            or not AI_BUTTONS_ENABLED
            or academic_session_id is None
        ):
            return None
    ...
    async def _save_academic_response_to_project(
        ...
        *,
        ...
        if session is None:
            return "Não encontrei essa sessão acadêmica."
        try:
            ...
            return "Não consegui salvar esta resposta no projeto agora."
        return (
