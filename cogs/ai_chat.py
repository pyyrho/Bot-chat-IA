...
CEREBRAS_MODELS = [
    "llama-3.3-70b",       # 70B robusto — modelo principal
    "llama3.1-8b",         # rápido e leve — fallback
]
...
SAMBANOVA_MODELS = [
    "DeepSeek-R1",                       # DeepSeek R1 671B completo — melhor raciocínio
    "DeepSeek-V3.1",                     # DeepSeek V3.1 — rápido e capaz para chat geral
    "Meta-Llama-3.3-70B-Instruct",       # robusto, bom fallback
    "Meta-Llama-3.1-405B-Instruct",      # 405B no free tier
    "Meta-Llama-3.1-70B-Instruct",       # alternativa leve
...
class AIChat(commands.Cog):
    ...
    #  Texto:  Cerebras → SambaNova → Groq → OpenRouter
    #          (Groq fica por último para preservar as 5 chaves)
    # ══════════════════════════════════════════════════════════
    ...
    async def get_ai_response(self, user_message: str, user_id: int, user_name: str,
        ...
        if image_data:
            ...
            # Texto: waterfall Cerebras → SambaNova → Groq → OpenRouter
            # Groq fica por penúltimo; OpenRouter é o último recurso
            logger.info(f"Tipo detectado: [{query_type}] → waterfall Cerebras → SambaNova → Groq → OpenRouter")
            ...
            if reply is None:
                logger.info("SambaNova esgotado — usando Groq (reserva)")
    ...
    async def parse_mod_command(self, message: str, mentions: list) -> dict:
        ...
        try:
            # Preserva Groq — usa Cerebras/SambaNova/OpenRouter primeiro
    ...
    @app_commands.command(name="testar-ia", description="🧪 Testa um provedor/modelo específico [OWNER]")
    @app_commands.describe(
        provedor="Qual provedor testar",
        modelo="Nome exato do modelo (opcional — usa o primeiro da lista se omitido)",
    ...
    async def testar_ia(self, interaction: discord.Interaction, provedor: str,
        ...
        try:
            ...
            resultado = f"❌ **Erro em {provedor} `{modelo_usado}`**\n`{type(e).__name__}: {e}`"

        embed = discord.Embed(title="🧪 Teste de IA", description=resultado, color=discord.Color.purple())
