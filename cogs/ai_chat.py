...
def _normalize_ai_output(text: str) -> str:
    ...
    # de tokens está apertado. Só converte "\n" em " " quando o caractere anterior
    # não é outro "\n" (senão seria parágrafo real) e o próximo não é "#", "-",
    # "*", "`" ou dígito (senão seria markdown de lista/título/código).
    def _fix_mid_paragraph_breaks(m: re.Match) -> str:
        before = m.group(1)
        after = m.group(2)
        # Preserva parágrafo real (linha anterior termina com \n) e listas/títulos
        if not after or after[0] in ("#", "-", "*", "`", ">", "|", "0123456789"):
            return before + "\n" + after
        # Preserva se a linha anterior termina com dois pontos (lista a seguir)
        before_stripped = before.rstrip()
        if before_stripped.endswith(":"):
            return before + "\n" + after
        return before + " " + after

    # Apenas colapsa \n simples (não precedidos de \n) em espaço quando seguro
