...
CEREBRAS_MODELS = [
    "gpt-oss-120b",                      # GPT OSS 120B — mais capaz, produção (~3000 tok/s)
    "qwen-3-235b-a22b-instruct-2507",    # Qwen 3 235B — preview, excelente raciocínio
...
class AIChat(commands.Cog):
    ...
    async def status_ia(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Cerebras", value=status(cerebras_count), inline=True)
        embed.add_field(name="SambaNova", value=status(sambanova_count), inline=True)
        embed.add_field(name="Groq", value=status(groq_count), inline=True)
        embed.add_field(name="OpenRouter", value=status(or_count), inline=True)
        embed.add_field(
            name="Visão (imagens)",
        ...
        embed.add_field(name="Modelos — SambaNova", value="\n".join([f"`{m}`" for m in SAMBANOVA_MODELS]), inline=False)
        embed.add_field(name="Modelos — Groq", value="\n".join([f"`{m}`" for m in GROQ_MODELS]), inline=False)

        embed.set_footer(text="Ordem de chamada: Cerebras → SambaNova → Groq → OpenRouter")
        await interaction.response.send_message(embed=embed, ephemeral=True)
