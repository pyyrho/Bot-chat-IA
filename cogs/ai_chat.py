...
class AIChat(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        ...
        for env_name in ("GROQ_API_KEY", "GROQ_API_KEY_2", "GROQ_API_KEY_3", "GROQ_API_KEY_4", "GROQ_API_KEY_5"):
            ...
            if key:
                # max_retries=0 evita esperas longas internas da biblioteca quando uma chave bate limite.
                # O próprio Revolux faz a rotação imediata entre as chaves.
                try:
                    client = Groq(api_key=key, max_retries=0, timeout=30.0)
                except TypeError:
    ...
    async def _call_groq(
        ...
        *,
        ...
        """Chama a Groq com rotação correta: modelo principal em todas as chaves, depois fallback.

        Ordem desejada:
        1. openai/gpt-oss-120b na chave 1, 2, 3, 4 e 5.
        2. Se todas baterem limite/erro, tenta llama-3.3-70b-versatile nas chaves 1, 2, 3, 4 e 5.

        429 é limite temporário de chave/conta, não modelo indisponível. Por isso ele é tratado
        antes de qualquer detecção de erro de modelo.
        """
        if not self.groq_clients:
            return None

        model_order = models or GROQ_MODELS
        total_keys = len(self.groq_clients)

        for model in model_order:
            start_key_index = self.groq_index % total_keys
            logger.info("Tentando modelo Groq %s em até %s chave(s).", model, total_keys)

            for offset in range(total_keys):
                client_index = (start_key_index + offset) % total_keys
                client = self.groq_clients[client_index]
                ...
                try:
                    ...
                    if is_limit:
                        logger.warning(
                            "Limite na chave Groq #%s usando %s. Tentando próxima chave.",
                            key_number,
                            model,
                        )
                        self.groq_index = (client_index + 1) % total_keys
                        continue

                    if is_auth_error:
                        logger.error(
                            "Chave Groq #%s inválida/bloqueada usando %s. Tentando próxima chave.",
                            key_number,
                            model,
                        )
                        self.groq_index = (client_index + 1) % total_keys
                    ...
                    if is_model_error:
                        logger.warning("Modelo indisponível no Groq: %s. Indo para o próximo modelo.", model)
                        break

                    logger.error(
                        "Erro Groq chave #%s modelo %s: %s: %s",
                        key_number,
                        model,
                        type(exc).__name__,
                        exc,
            ...
            logger.warning("Todas as chaves falharam para o modelo Groq %s. Tentando próximo modelo.", model)

        logger.error("Todos os modelos e chaves Groq falharam nesta requisição.")
        return None
    ...
    async def _send_long_reply(self, message: discord.Message, text: str) -> None:
        """Envia resposta longa sem quebrar se a mensagem original sumir.

        O erro "In message_reference: Unknown message" acontece quando o Discord não aceita
        mais responder como reply. Nesses casos, o bot envia no canal normalmente.
        """
        safe_text = (text or "").strip() or "Não consegui gerar uma resposta agora."
        chunks: list[str] = []
        current = ""
