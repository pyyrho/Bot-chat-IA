...
NORMAL_MAX_TOKENS = max(256, min(_safe_int(os.getenv("AI_NORMAL_MAX_TOKENS"), 800), 4096))
ACADEMIC_MAX_TOKENS = max(384, min(_safe_int(os.getenv("AI_ACADEMIC_MAX_TOKENS"), 1200), 4096))
DEEP_MAX_TOKENS = max(512, min(_safe_int(os.getenv("AI_DEEP_MAX_TOKENS"), 1600), 6144))
MAX_CONTINUATIONS = max(0, min(_safe_int(os.getenv("AI_MAX_CONTINUATIONS"), 0), 3))
...
- REGRA PRINCIPAL: entregue uma resposta COMPLETA e CURTA. É melhor uma resposta menor e completa do que uma grande e cortada no meio.
- Perguntas simples ou casuais: responda em 1 a 4 frases. Não elabore além do necessário.
- Perguntas acadêmicas ou complexas: responda em no máximo 3 a 5 parágrafos curtos. Se o tema for grande, cubra os pontos principais e ofereça aprofundar se o usuário quiser.
- NUNCA comece uma resposta que não consegue terminar. Se o assunto for grande, escreva uma versão concisa e completa.
- Termine a resposta inteira. Nunca pare no meio de uma frase, item, título, citação, negrito ou bloco de código.
- Prefira parágrafos curtos e listas simples apenas quando realmente organizarem a resposta.
- Não use fragmentos soltos, títulos quebrados ou numeração sem conteúdo.
- NUNCA use tabelas Markdown (| col | col |) em respostas filosóficas, acadêmicas, de lógica ou matemáticas. Tabelas só são aceitáveis se o usuário pedir explicitamente uma comparação em formato de tabela.
- NUNCA use separadores horizontais (---, ===, ___) no corpo do texto.
- NUNCA use headers pesados (##, ###) em respostas acadêmicas ou filosóficas. Se precisar indicar seções, use numeração simples ("1.", "2.") ou negrito seguido de dois-pontos.
...
1. Dê a resposta direta primeiro, em 1 a 2 frases.
2. Defina termos importantes inline, no próprio parágrafo — nunca em tabela.
3. Explique o raciocínio em parágrafos curtos ou etapas numeradas simples (máximo 4 etapas).
4. Diferencie fato, interpretação, opinião e especulação quando relevante.
5. Mostre objeções ou exceções APENAS se forem centrais para a resposta.
...
REGRA DE OURO DO MODO ACADÊMICO: prefira uma resposta de 2 parágrafos completos a uma resposta de 6 parágrafos cortada no meio. Se o usuário quiser mais profundidade, ele vai pedir.
FORMATO ABSOLUTO NO MODO ACADÊMICO: prosa contínua ou parágrafos naturais. Sem tabelas, sem separadores, sem headers Markdown. Listas simples (- item) apenas com moderação quando necessário organizar vários pontos distintos.
...
class AIChat(commands.Cog):
    projeto_academico = app_commands.Group(
        name="projeto-academico",
        description="Workspace persistente para pesquisas, fontes, notas e decisões.",
    )
    ...
    async def _maybe_build_external_context(
        ...
        should_search = mode == "search" or (analysis.needs_search if analysis else academic and ACADEMIC_SEARCH_ENABLED)
    ...
    async def get_ai_response(
        ...
        *,
        ...
        # Uma requisição por usuário de cada vez preserva a ordem do histórico e
        # impede duas extrações de memória concorrentes para a mesma conversa.
        async with self.request_locks[user_id]:
            ...
            if memory_context:
                ...
                if memory_prompt:
                    user_content += (
                        "\n\nMEMÓRIA RELEVANTE DO USUÁRIO. Use somente quando ajudar e não "
                        "trate lembranças incertas como fatos absolutos:\n" + memory_prompt
                    )
            ...
            if quality.likely_cut_off:
                logger.warning(
                    "Trace %s ainda parece truncada | modelo=%s | fim=%s | continuações=%s",
                    trace_id,
                    model_result.model,
            ...
            self.conversation_store.append_exchange(user_id, clean_user_message, reply)
            try:
    ...
    @app_commands.choices(modo=[
        ...
        app_commands.Choice(name="Diagnóstico de conhecimento", value="diagnostico"),
        app_commands.Choice(name="Método socrático", value="socratico"),
        app_commands.Choice(name="Flashcards", value="flashcards"),
        app_commands.Choice(name="Revisão espaçada", value="revisao"),
    ])
    async def estudar_command(self, interaction: discord.Interaction, tema: str, modo: str, privado: bool = False) -> None:
        ...
        instructions = {
            ...
            "diagnostico": "Crie um diagnóstico gradual sem presumir domínio: perguntas, rubrica e trilha recomendada. Não dê o gabarito antes das perguntas.",
            "socratico": "Ensine pelo método socrático: faça perguntas encadeadas, revele pressupostos e ofereça pistas graduais em vez de entregar tudo de imediato.",
            "flashcards": "Crie flashcards atômicos, sem ambiguidades, com pergunta na frente, resposta curta no verso e uma explicação opcional.",
            "revisao": "Monte uma revisão espaçada em blocos: hoje, 1 dia, 3 dias, 7 dias e 21 dias, com recuperação ativa e critérios de domínio.",
        }
        response = await self.get_ai_response(
            tema,
            interaction.user.id,
    ...
    async def analisar_argumento_command(self, interaction: discord.Interaction, argumento: str, privado: bool = False) -> None:
        await interaction.response.defer(ephemeral=privado, thinking=True)
        local_map = map_argument(argumento)
        response = await self.get_ai_response(
            argumento,
            interaction.user.id,
            interaction.user.display_name,
            ...
            extra_instruction=(
                "Extraia premissas, conclusão, forma lógica quando possível, validade, solidez, "
                "falácias e uma versão mais forte do argumento. Use o mapa heurístico abaixo apenas "
                "como ponto de partida e corrija-o quando necessário:\n" + local_map.compact()
            ),
        )
        view = self._make_view(response, argumento)
        await self._send_interaction_followup(interaction, response, ephemeral=privado, view=view)

    @app_commands.command(name="pesquisar", description="Pesquisa acadêmica multibase com rastreamento de evidências.")
    @app_commands.describe(
        tema="Pergunta ou tema da pesquisa.",
        profundidade="Nível de aprofundamento.",
        privado="Se ativado, somente você vê a resposta.",
    )
    @app_commands.choices(profundidade=[
        app_commands.Choice(name="Rápida", value="rapida"),
        app_commands.Choice(name="Profunda", value="profunda"),
        app_commands.Choice(name="Revisão estruturada", value="revisao"),
    ])
    async def pesquisar_command(
        self,
        interaction: discord.Interaction,
        tema: str,
        profundidade: str = "profunda",
        privado: bool = False,
    ) -> None:
        await interaction.response.defer(ephemeral=privado, thinking=True)
        instructions = {
            "rapida": (
                "Faça uma síntese acadêmica objetiva. Use as fontes numeradas [S1], [S2] "
                "somente quando sustentarem a frase e exponha lacunas de evidência."
            ),
            "profunda": (
                "Produza análise profunda: resposta direta, conceitos, evidências a favor e contra, "
                "consenso ou controvérsia, limitações e conclusão calibrada. Cite [S1], [S2]."
            ),
            "revisao": (
                "Estruture como revisão de literatura compacta: questão, estratégia de busca, tipos "
                "de fonte, achados convergentes, divergências, lacunas, limitações e referências. "
                "Não chame de revisão sistemática formal sem protocolo e triagem completos."
            ),
        }
        prompt = f"Pesquise academicamente e responda: {tema}"
        response = await self.get_ai_response(
            prompt,
            interaction.user.id,
            interaction.user.display_name,
            guild_id=interaction.guild_id,
            channel_id=interaction.channel_id,
            forced_query_type="academic",
            extra_instruction=instructions.get(profundidade, instructions["profunda"]),
        )
        await self._send_interaction_followup(
            interaction,
            response,
            ephemeral=privado,
            view=self._make_view(response, tema),
    ...
    @app_commands.command(name="verificar-afirmacao", description="Verifica uma afirmação e mostra o grau de sustentação acadêmica.")
    @app_commands.describe(afirmacao="Afirmação que será verificada.", privado="Se ativado, somente você vê a resposta.")
    async def verificar_afirmacao_command(
        self,
        interaction: discord.Interaction,
        afirmacao: str,
        privado: bool = False,
        ...
        await interaction.response.defer(ephemeral=privado, thinking=True)
        response = await self.get_ai_response(
            f"Verifique esta afirmação com fontes acadêmicas: {afirmacao}",
            interaction.user.id,
            interaction.user.display_name,
            guild_id=interaction.guild_id,
            channel_id=interaction.channel_id,
            forced_query_type="academic",
            extra_instruction=(
                "Dê um veredito explícito entre: confirmada, parcialmente confirmada, controversa, "
                "inconclusiva ou provavelmente incorreta. Decomponha afirmações compostas, cite "
                "[S1], [S2], diferencie correlação de causalidade e informe a confiança."
            ),
        )
        await self._send_interaction_followup(
            interaction,
            response,
            ephemeral=privado,
            view=self._make_view(response, afirmacao),
        )

    @app_commands.command(name="comparar-teorias", description="Compara teorias, autores ou explicações sem criar falsa equivalência.")
    @app_commands.describe(itens="Teorias ou autores que deseja comparar.", criterio="Critério principal da comparação.", privado="Se ativado, somente você vê.")
    async def comparar_teorias_command(
        self,
        interaction: discord.Interaction,
        itens: str,
        criterio: str = "fundamentos, argumentos, evidências, limites e aplicações",
        privado: bool = False,
    ) -> None:
        await interaction.response.defer(ephemeral=privado, thinking=True)
        response = await self.get_ai_response(
            f"Compare academicamente {itens}. Critério: {criterio}",
            interaction.user.id,
            interaction.user.display_name,
            guild_id=interaction.guild_id,
            channel_id=interaction.channel_id,
            forced_query_type="academic",
            extra_instruction=(
                "Defina cada posição em seus próprios termos, use uma matriz comparativa quando "
                "ajudar, apresente melhores argumentos e objeções de cada lado, explique onde a "
                "comparação deixa de ser simétrica e cite fontes numeradas."
            ),
        )
        await self._send_interaction_followup(
            interaction,
            response,
            ephemeral=privado,
            view=self._make_view(response, itens),
        )

    @app_commands.command(name="referencias", description="Busca referências reais e formata em ABNT, APA, Chicago, Vancouver ou BibTeX.")
    @app_commands.describe(tema="Tema das referências.", estilo="Estilo bibliográfico.", quantidade="Quantidade entre 3 e 15.")
    @app_commands.choices(estilo=[
        app_commands.Choice(name="ABNT", value="abnt"),
        app_commands.Choice(name="APA", value="apa"),
        app_commands.Choice(name="Chicago", value="chicago"),
        app_commands.Choice(name="Vancouver", value="vancouver"),
        app_commands.Choice(name="BibTeX", value="bibtex"),
    ])
    async def referencias_command(
        self,
        interaction: discord.Interaction,
        tema: str,
        estilo: str = "abnt",
        quantidade: app_commands.Range[int, 3, 15] = 8,
    ) -> None:
        await interaction.response.defer(ephemeral=True, thinking=True)
        query = f"pesquise artigos acadêmicos sobre {tema}"
        analysis = self.query_router.analyze(query, "academic")
        analysis.needs_search = True
        analysis.needs_sources = True
        result = await self.academic_orchestrator.search(query, analysis)
        if not result.works:
            await interaction.followup.send(
                "<:1000032056:1507947210057322637> Não encontrei metadados acadêmicos confiáveis para esse tema.",
                ephemeral=True,
            )
            return
        text = f"## Referências sobre {tema}\n\n" + bibliography(result.works, estilo, quantidade)
        text += (
            f"\n\n-# Bases consultadas: {', '.join(result.diagnostics.providers_succeeded) or 'nenhuma'} "
            f"· {result.diagnostics.elapsed_ms} ms · resultados deduplicados: {len(result.works)}"
        )
        chunks = _split_discord_text(text, limit=DISCORD_TEXT_LIMIT)
        for chunk in chunks:
            await interaction.followup.send(chunk, ephemeral=True)

    @app_commands.command(name="analisar-documento", description="Analisa PDF ou texto com resumo, fichamento, crítica ou mapa de argumentos.")
    @app_commands.describe(
        arquivo="PDF ou arquivo textual de até o limite configurado.",
        modo="Tipo de análise.",
        foco="Pergunta ou foco opcional.",
        privado="Se ativado, somente você vê a resposta.",
    )
    @app_commands.choices(modo=[
        app_commands.Choice(name="Resumo analítico", value="resumo"),
        app_commands.Choice(name="Fichamento", value="fichamento"),
        app_commands.Choice(name="Crítica metodológica", value="critica"),
        app_commands.Choice(name="Mapa de argumentos", value="argumentos"),
    ])
    async def analisar_documento_command(
        self,
        interaction: discord.Interaction,
        ...
        await interaction.response.defer(ephemeral=privado, thinking=True)
        if arquivo.size and arquivo.size > self.document_processor.max_bytes:
            await interaction.followup.send(
                f"<:1000032056:1507947210057322637> O arquivo excede o limite de "
                f"{self.document_processor.max_bytes // 1024 // 1024} MB.",
                ephemeral=privado,
            )
            return
        try:
            ...
            await interaction.followup.send(
                f"<:1000032056:1507947210057322637> Não consegui processar o arquivo: `{_compact_exception_message(exc)}`",
                ephemeral=privado,
            )
            return
        if not extraction.text.strip():
            warning = "; ".join(extraction.warnings or []) or "nenhum texto extraível foi encontrado"
            await interaction.followup.send(
                f"<:1000032056:1507947210057322637> {warning}.",
                ephemeral=privado,
            )
            return
        focus = foco or "estrutura, conceitos, argumentos, evidências e limitações"
        compact = self.document_processor.compact_for_prompt(extraction.text, focus=focus)
        instructions = {
            "resumo": "Resuma de forma analítica: tese, estrutura, conceitos, evidências, conclusão e limitações.",
            "fichamento": "Produza fichamento acadêmico: referência disponível, tema, problema, tese, conceitos, argumentos, citações indiretas e avaliação crítica.",
            "critica": "Faça crítica metodológica justa: desenho, amostra, medidas, inferências, vieses, validade, replicabilidade e o que os dados não permitem concluir.",
            "argumentos": "Mapeie tese, premissas explícitas e implícitas, conclusões intermediárias, objeções, respostas, ambiguidades e força inferencial.",
        }
        prompt = (
            f"Documento: {arquivo.filename}\nFoco: {focus}\n\n"
            f"CONTEÚDO EXTRAÍDO:\n{compact}"
        )
        response = await self.get_ai_response(
            prompt,
            ...
            guild_id=interaction.guild_id,
            channel_id=interaction.channel_id,
            forced_query_type="study" if modo in {"resumo", "fichamento"} else "argument",
            extra_instruction=instructions.get(modo, instructions["resumo"]),
        )
        if extraction.truncated:
            response.used_tools.append("documento compactado")
        await self._send_interaction_followup(
            interaction,
            response,
            ephemeral=privado,
            view=self._make_view(response, f"Analisar {arquivo.filename}"),
        )

    @projeto_academico.command(name="criar", description="Cria e ativa um workspace de pesquisa.")
    async def projeto_criar(
        self,
        interaction: discord.Interaction,
        nome: str,
        objetivo: str = "",
        ...
        await interaction.response.send_message(
            f"<:1000032055:1507947171624910859> Projeto `#{project.id}` **{project.name}** criado e ativado.",
            ephemeral=True,
        )

    @projeto_academico.command(name="listar", description="Lista seus projetos acadêmicos neste servidor.")
    async def projeto_listar(self, interaction: discord.Interaction) -> None:
        projects = await self.research_workspace.list(interaction.user.id, interaction.guild_id)
        if not projects:
            await interaction.response.send_message("Você ainda não criou projetos acadêmicos aqui.", ephemeral=True)
            return
        profile = await self._get_user_profile(interaction.user.id)
        active = profile.get("active_project_id")
        lines = ["## Projetos acadêmicos"]
        for project in projects:
            marker = " · **ativo**" if project.id == active else ""
            lines.append(f"`#{project.id}` **{project.name}**{marker}\n{project.objective[:240] or 'Sem objetivo registrado.'}")
        await interaction.response.send_message("\n\n".join(lines)[:3900], ephemeral=True)

    @projeto_academico.command(name="ativar", description="Ativa um projeto para ser usado automaticamente nas conversas.")
    async def projeto_ativar(self, interaction: discord.Interaction, projeto_id: int) -> None:
        activated = await self.research_workspace.set_active(
            interaction.user.id,
            interaction.guild_id,
            projeto_id,
        )
        if not activated:
            await interaction.response.send_message("Projeto não encontrado entre os seus projetos deste servidor.", ephemeral=True)
            return
        profile = await self._get_user_profile(interaction.user.id)
        profile["active_project_id"] = projeto_id
        await self._save_user_profile(interaction.user.id, profile)
        await interaction.response.send_message(f"Projeto `#{projeto_id}` ativado.", ephemeral=True)

    @projeto_academico.command(name="nota", description="Adiciona uma nota, decisão ou questão ao projeto ativo.")
    @app_commands.choices(tipo=[
        app_commands.Choice(name="Nota", value="note"),
        app_commands.Choice(name="Decisão", value="decision"),
        app_commands.Choice(name="Questão de pesquisa", value="question"),
    ])
    async def projeto_nota(
        self,
        interaction: discord.Interaction,
        titulo: str,
        conteudo: str,
        ...
        profile = await self._get_user_profile(interaction.user.id)
        selected = projeto_id or profile.get("active_project_id")
        if not selected:
            await interaction.response.send_message("Crie ou ative um projeto primeiro.", ephemeral=True)
            return
        ...
        if not item:
            await interaction.response.send_message("Projeto não encontrado.", ephemeral=True)
            return
        await interaction.response.send_message(f"Item `#{item.id}` adicionado ao projeto `#{selected}`.", ephemeral=True)

    @projeto_academico.command(name="fonte", description="Pesquisa e adiciona a fonte acadêmica mais relevante ao projeto.")
    async def projeto_fonte(
        self,
        interaction: discord.Interaction,
        consulta: str,
        projeto_id: Optional[int] = None,
        ...
        selected = projeto_id or profile.get("active_project_id")
        if not selected:
            await interaction.followup.send("Crie ou ative um projeto primeiro.", ephemeral=True)
            return
        analysis = self.query_router.analyze("pesquise artigos " + consulta, "academic")
        analysis.needs_search = True
        result = await self.academic_orchestrator.search(consulta, analysis)
        if not result.works:
            await interaction.followup.send("Nenhuma fonte confiável foi encontrada.", ephemeral=True)
            return
        work = result.works[0]
        item = await self.research_workspace.add_item(
            interaction.user.id,
            ...
            item_type="source",
            title=work.title,
            content=work.compact(abstract_chars=1200),
            url=work.url,
        )
        if not item:
            ...
            return
        await interaction.followup.send(
            f"<:1000032055:1507947171624910859> Fonte adicionada ao projeto `#{selected}`:\n**{work.title}**\n{work.url or work.doi}",
            ephemeral=True,
        )

    @projeto_academico.command(name="ver", description="Mostra objetivo, notas e fontes de um projeto.")
    async def projeto_ver(
        self,
        interaction: discord.Interaction,
        projeto_id: Optional[int] = None,
    ) -> None:
        ...
        selected = projeto_id or profile.get("active_project_id")
        if not selected:
            await interaction.response.send_message("Nenhum projeto foi indicado ou ativado.", ephemeral=True)
            return
        project = await self.research_workspace.get(
            interaction.user.id,
            interaction.guild_id,
            int(selected),
        )
        if not project:
            ...
            return
        await interaction.response.send_message(
            ("## " + project.name + "\n" + project.as_context(max_chars=3600))[:3900],
            ephemeral=True,
        )

    @projeto_academico.command(name="sintetizar", description="Analisa o projeto ativo e indica achados, conflitos, lacunas e próximos passos.")
    async def projeto_sintetizar(
        self,
        interaction: discord.Interaction,
        projeto_id: Optional[int] = None,
        privado: bool = True,
        ...
        selected = projeto_id or profile.get("active_project_id")
        if not selected:
            await interaction.followup.send("Nenhum projeto foi indicado ou ativado.", ephemeral=privado)
            return
        project = await self.research_workspace.get(interaction.user.id, interaction.guild_id, int(selected))
        if not project:
            await interaction.followup.send("Projeto não encontrado.", ephemeral=privado)
            return
        response = await self.get_ai_response(
            "Analise o workspace acadêmico ativo e produza uma síntese crítica.",
            interaction.user.id,
            interaction.user.display_name,
            guild_id=interaction.guild_id,
            channel_id=interaction.channel_id,
            forced_query_type="academic",
            extra_instruction=(
                "Use estritamente o workspace fornecido. Organize em: questão/objetivo, achados, "
                "convergências, conflitos, pressupostos, lacunas, qualidade das fontes e próximos "
                "passos. Não invente conteúdo ausente."
            ),
        )
        await self._send_interaction_followup(
            interaction,
            response,
            ephemeral=privado,
            view=self._make_view(response, f"Síntese do projeto #{selected}"),
        )

    @projeto_academico.command(name="exportar", description="Exporta o projeto em Markdown, com notas, fontes e decisões.")
    async def projeto_exportar(
        self,
        interaction: discord.Interaction,
        projeto_id: Optional[int] = None,
    ) -> None:
        ...
        if not project:
            await interaction.response.send_message("Projeto não encontrado.", ephemeral=True)
            return
        content = self.research_workspace.export_markdown(project).encode("utf-8")
        safe_name = re.sub(r"[^a-zA-Z0-9_-]+", "-", project.name).strip("-")[:60] or f"projeto-{project.id}"
        file = discord.File(io.BytesIO(content), filename=f"{safe_name}.md")
        await interaction.response.send_message(
            f"<:1000032055:1507947171624910859> Projeto `#{project.id}` exportado.",
            file=file,
            ephemeral=True,
        )

    @projeto_academico.command(name="apagar", description="Apaga permanentemente um projeto acadêmico seu.")
    async def projeto_apagar(
        self,
        interaction: discord.Interaction,
        projeto_id: int,
        confirmacao: str,
        ...
        if confirmacao.strip().upper() != "APAGAR":
            await interaction.response.send_message(
                "Operação cancelada. Digite `APAGAR` no campo de confirmação para excluir.",
                ephemeral=True,
            )
            return
        removed = await self.research_workspace.delete(interaction.user.id, interaction.guild_id, projeto_id)
        if not removed:
            await interaction.response.send_message("Projeto não encontrado.", ephemeral=True)
            return
        profile = await self._get_user_profile(interaction.user.id)
        if int(profile.get("active_project_id") or 0) == projeto_id:
            profile.pop("active_project_id", None)
            await self._save_user_profile(interaction.user.id, profile)
        await interaction.response.send_message(
            f"Projeto `#{projeto_id}` apagado permanentemente.",
            ephemeral=True,
        )
