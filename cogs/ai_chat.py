...
_THINKING_BUDGET_BY_LEVEL = {"low": 1024, "medium": 4096, "high": 8192}


def _build_thinking_config(model: str, thinking_level: str) -> Any:
    """Monta o ThinkingConfig correto para o modelo Gemini em uso.

    Modelos da família Gemini 2.5 (ex.: gemini-2.5-flash) não aceitam o
    parâmetro `thinking_level` (erro 400 "Thinking level is not supported
    for this model") — eles usam `thinking_budget` (int). Já os modelos
    Gemini 3 usam `thinking_level` ("low"/"medium"/"high"). Detectamos pelo
    nome do modelo e escolhemos o campo certo.
    """
    level = thinking_level if thinking_level in _THINKING_BUDGET_BY_LEVEL else "low"
    if genai_types is None:
        return None
    if "2.5" in model or "1.5" in model:
        return genai_types.ThinkingConfig(
            thinking_budget=_THINKING_BUDGET_BY_LEVEL[level],
...
def classify_provider_error(exc: Exception) -> ProviderErrorInfo:
    # Algumas libs (ex.: google-genai) usam `exc.status` para o nome do status
    # (ex.: "INVALID_ARGUMENT") e `exc.code`/`exc.status_code` para o código
    # numérico HTTP. Normalizamos para sempre obter um int (ou None), evitando
    # comparações inválidas como `status >= 500` quando status é string.
    ...
    if isinstance(code_raw, int):
        ...
        status = status_raw
    elif isinstance(status_raw, str) and status_raw.strip().isdigit():
    ...
    async def _gemini_generate_once(
        ...
        *,
        ...
        config = genai_types.GenerateContentConfig(
            ...
            top_p=0.9,
            thinking_config=_build_thinking_config(GEMINI_MODEL, thinking_level),
        )
