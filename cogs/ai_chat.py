...
# ATENÇÃO: gpt-oss-120b e zai-glm-4.7 retornam 404 — removidos
CEREBRAS_MODELS = [
    "qwen-3-235b-a22b-instruct-2507",    # Qwen 3 235B — principal, excelente raciocínio
    "llama-3.3-70b",                      # Llama 3.3 70B — fallback capaz
    "llama3.1-70b",                       # Llama 3.1 70B — alternativa sólida
]
...
class AIChat(commands.Cog):
    ...
    async def get_ai_response(self, user_message: str, user_id: int, user_name: str,
        ...
        if image_data:
            ...
            if query_type == "news":
                # Notícias: Groq (capaz de seguir instrução anti-alucinação) → Cerebras → OpenRouter
                # Groq vai primeiro pois se Cerebras der rate limit cai em modelo fraco (llama3.1-8b)
                logger.info(f"Tipo detectado: [news] → waterfall Groq → Cerebras → OpenRouter")
                ...
                if reply is None:
                    logger.info("Cerebras esgotado — tentando OpenRouter (último recurso)")
                    preferred = select_openrouter_models(query_type, has_image=False)
                    reply = await self._call_openrouter(messages, max_tokens, temperature, None, preferred)
            else:
                # Chat normal / código: Cerebras → Groq → OpenRouter
