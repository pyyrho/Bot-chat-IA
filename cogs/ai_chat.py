...
from academic.citations import bibliography
...
AI_CHAT_BUILD = "2026-08-12-rotacao-memoria-bloqueio-v4"
...
class AIChat(commands.Cog):
    ...
    async def _maybe_build_external_context(
        ...
        if academic and ACADEMIC_SEARCH_ENABLED:
            ...
            # SEP e biblioteca local são valiosos, mas não devem atrasar toda
            # pergunta acadêmica. O complemento legado só entra para humanidades,
            # quando existe biblioteca local, ou por opção explícita no Railway.
            folded = _normalize_key(user_message)
            philosophy_markers = (
                "filosof", "epistemolog", "ontolog", "metafis", "etica", "logica",
                "kant", "platao", "aristoteles", "hegel", "nietzsche", "foucault",
            )
            library_exists = any(
            ...
            legacy_needed = force_legacy or library_exists or any(marker in folded for marker in philosophy_markers)
            if legacy_needed:
            ...
            if not isinstance(legacy_raw, Exception) and legacy_raw:
                ...
                contexts.append("FONTES COMPLEMENTARES (SEP/biblioteca/web acadêmica):\n" + legacy_ranked)
                sources.extend(self.source_ranker.source_labels(legacy_ranked, limit=6))
                tools.append("SEP/biblioteca acadêmica")

            combined = "\n\n".join(part for part in contexts if part)
            return combined[: max(ACADEMIC_CONTEXT_MAX_CHARS_DEFAULT * 3, 12000)], sources[:18], list(dict.fromkeys(tools)), cache_hit
    ...
    async def get_ai_response(
        ...
        *,
        ...
        analysis.needs_memory = _requests_persistent_memory(user_message)
        mode = analysis.mode
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        # clean_content resolve menções para nomes legíveis, permitindo que o
        # modelo diferencie participantes em vez de receber apenas <@123...>.
        content = getattr(message, "clean_content", "") or message.content
        if self.bot.user:
            for bot_reference in (
                f"<@{self.bot.user.id}>",
                f"<@!{self.bot.user.id}>",
        ...
        reply_context = self._reply_context_text(referenced_message)
        ...
        view = self._make_view(response, content)
