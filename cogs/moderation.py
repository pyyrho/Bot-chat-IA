...
class Moderation(commands.Cog):
    ...
    async def get_warn_count(self, guild_id: int, user_id: int) -> int:
        row = await db.pool.fetchrow(
            "SELECT COUNT(*) as c FROM warnings WHERE guild_id = $1 AND user_id = $2",
            guild_id, user_id
        )
    ...
    async def ai_analyze_message(self, content: str) -> dict:
        try:
            response = await asyncio.to_thread(
                ...
                max_tokens=150,
                system="""Você é um moderador de Discord. Analise a mensagem e retorne JSON:
{"violation": true/false, "severity": "low/medium/high", "reason": "motivo curto em português", "action": "warn/mute/kick/ban/none"}
Considere: hate speech, ameaças, conteúdo sexual explícito, bullying severo, spam malicioso.
Seja equilibrado — não puna conversas normais. Retorne APENAS o JSON.""",
                messages=[{"role": "user", "content": f"Mensagem: {content}"}]
            )
            import json
    ...
    @app_commands.describe(
        canal_log="Canal para logs de moderação",
        limite_avisos="Avisos antes de punição automática",
        acao_avisos="Ação ao atingir limite de avisos"
    )
    ...
    async def config_mod(self, interaction: discord.Interaction,
        ...
        await self.get_config(interaction.guild_id)  # garante que existe
        updates = {}
        if canal_log:
            updates["log_channel"] = canal_log.id
        if limite_avisos:
            updates["warn_threshold"] = max(1, limite_avisos)
        if acao_avisos:
            updates["warn_action"] = acao_avisos
        if updates:
    ...
    @app_commands.describe(palavra="Palavra para adicionar ou remover", acao="Adicionar ou remover")
