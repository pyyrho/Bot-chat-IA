...
Chave de API: MOD_GROQ_API_KEY (somente para moderação por IA)
"""
...
AI_DEEP_MODEL    = os.getenv("MOD_GROQ_DEEP_MODEL", "openai/gpt-oss-safeguard-20b")   # análise profunda via Groq
...
class Moderation(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        # Moderação por IA usa Groq (gpt-oss-safeguard-20b) exclusivo de moderação.
    ...
    async def _groq_chat(self, *, model: str, messages: list[dict], max_tokens: int = 250, temperature: float = 0.05) -> str:
        """
        ...
        async with aiohttp.ClientSession() as session:
            for _ in range(total):
                key = self._groq_keys[self._groq_index % total]
                self._groq_index = (self._groq_index + 1) % total
                try:
                    ...
                    last_exc = exc
                    logger.warning("Falha em chamada Groq (moderação); tentando próxima chave: %s", exc)
                    await asyncio.sleep(0.35)

        raise last_exc or RuntimeError("Falha desconhecida no Groq (moderação)")
