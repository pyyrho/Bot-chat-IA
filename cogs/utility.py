...
_HELP_PAGES: list[tuple[str, Callable[[], discord.Color], str]] = [
    ...
    (
        ...
        discord.Color.red,
        (
            "Todos os comandos de moderação são **slash commands** (`/`).\n\n"
    ...
    (
        ...
        (
            ...
            "→ `@Revolux bane @usuário [motivo]` · Banir\n"
            "→ `@Revolux muta @usuário [X] minutos [motivo]` · Silenciar\n"
            "→ `@Revolux desmuta @usuário` · Remover silêncio\n"
            "→ `@Revolux expulsa @usuário [motivo]` · Expulsar\n\n"
...
class Utility(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        # ── Tenta cada handler em ordem de especificidade ──
        handled = (
            await self._intent_userinfo(message, content, low)
            or await self._intent_avatar(message, content, low)
            or await self._intent_banner(message, content, low)
            or await self._intent_serverinfo(message, content, low)
            or await self._intent_roleinfo(message, content, low)
            or await self._intent_canalinfo(message, content, low)
            or await self._intent_poll(message, content, low)
            or await self._intent_lembrete(message, content, low)
            or await self._intent_timestamp(message, content, low)
            or await self._intent_cor(message, content, low)
            or await self._intent_resumir(message, content, low)
            or await self._intent_traduzir(message, content, low)
            or await self._intent_moderar(message, content, low)
        )
        # Se handled=True, o ai_chat não recebe (o bot.user já foi "consumido" da menção)
        # Se handled=False, o ai_chat captura normalmente pelo próprio on_message dele.
        # Não precisamos fazer nada aqui — a ordem de listeners no discord.py
        # permite que ambos ouçam, mas só o ai_chat chama get_ai_response se nós não
        # definirmos nenhum flag de bloqueio. Para evitar dupla resposta, adicionamos
        # um atributo temporário na mensagem quando processamos aqui.
        if handled:
            message.__dict__["_util_handled"] = True
    ...
    async def _intent_moderar(
        self, message: discord.Message, content: str, low: str
    ) -> bool:
        """Handler de moderação por menção: ban, mute, unmute, kick."""
        # Exige que o autor tenha permissões de moderação
        ...
        is_ban = any(t in low for t in ban_triggers)
        is_mute = any(t in low for t in mute_triggers)
        is_unmute = any(t in low for t in unmute_triggers)
        is_kick = any(t in low for t in kick_triggers)

        if not (is_ban or is_mute or is_unmute or is_kick):
            return False

        # Verifica permissão do autor
        if is_ban and not author.guild_permissions.ban_members:
            await self._send_error(message, "Você não tem permissão para banir membros.")
            return True
        if is_kick and not author.guild_permissions.kick_members:
            await self._send_error(message, "Você não tem permissão para expulsar membros.")
            return True
        if (is_mute or is_unmute) and not author.guild_permissions.moderate_members:
            await self._send_error(message, "Você não tem permissão para silenciar membros.")
            return True

        # Resolve o alvo (primeira menção que não seja o bot)
        target: Optional[discord.Member] = None
        for m in message.mentions:
            if m != self.bot.user:
                target = message.guild.get_member(m.id) if message.guild else None
                break

        if not target:
            await self._send_error(message, "Mencione o usuário que deseja moderar. Ex: `@Revolux bane @usuário motivo`")
            return True

        if target.top_role >= author.top_role and author.id != message.guild.owner_id:
            await self._send_error(message, f"Você não pode moderar {target.mention} pois ele tem cargo igual ou superior ao seu.")
            return True

        # Extrai motivo (texto após a menção do alvo)
        reason_match = re.search(r"<@!?\d+>\s*(.+)?$", content)
        reason = reason_match.group(1).strip() if reason_match and reason_match.group(1) else "Sem motivo informado"

        try:
            if is_ban:
                await target.ban(reason=f"{author} via menção: {reason}"[:512], delete_message_days=0)
                card = V2Card(accent_color=discord.Color.red())
                card.add_header(f"{EMOJI_MOD} Membro banido", subtitle=f"{target} foi banido do servidor.")
                card.add_text(f"**Usuário:** {target.mention} (`{target.id}`)\n**Motivo:** {_truncate(reason, 500)}\n**Moderador:** {author.mention}")
                card.add_footer()
                await self._send_card(message, card)

            elif is_kick:
                await target.kick(reason=f"{author} via menção: {reason}"[:512])
                card = V2Card(accent_color=discord.Color.orange())
                card.add_header(f"{EMOJI_MOD} Membro expulso", subtitle=f"{target} foi expulso do servidor.")
                card.add_text(f"**Usuário:** {target.mention} (`{target.id}`)\n**Motivo:** {_truncate(reason, 500)}\n**Moderador:** {author.mention}")
                card.add_footer()
                await self._send_card(message, card)

            elif is_unmute:
                await target.timeout(None, reason=f"{author} via menção: removeu silêncio")
                card = V2Card(accent_color=discord.Color.green())
                card.add_header(f"{EMOJI_MOD} Silêncio removido", subtitle=f"{target} pode falar novamente.")
                card.add_text(f"**Usuário:** {target.mention}\n**Moderador:** {author.mention}")
                card.add_footer()
                await self._send_card(message, card)

            elif is_mute:
                # Extrai duração em minutos: "X minutos" ou "X min"
                dur_match = re.search(r"(\d+)\s*(?:minutos?|min\b|m\b)", low)
                minutes = int(dur_match.group(1)) if dur_match else 10
                minutes = max(1, min(minutes, 40320))  # 1 min ~ 4 semanas
                until = discord.utils.utcnow() + __import__("datetime").timedelta(minutes=minutes)
                await target.timeout(until, reason=f"{author} via menção: {reason}"[:512])
                card = V2Card(accent_color=discord.Color.gold())
                card.add_header(f"{EMOJI_MOD} Membro silenciado")
                card.add_text(
                    f"**Usuário:** {target.mention} (`{target.id}`)\n"
                    f"**Duração:** {minutes} minuto(s)\n"
                    f"**Motivo:** {_truncate(reason, 500)}\n"
                    f"**Moderador:** {author.mention}"
                )
                card.add_footer()
                await self._send_card(message, card)

        except discord.Forbidden:
            await self._send_error(message, "Não tenho permissão para realizar essa ação. Verifique se meu cargo está acima do alvo.")
        except discord.HTTPException as exc:
            await self._send_error(message, f"Erro ao moderar: {exc}")
