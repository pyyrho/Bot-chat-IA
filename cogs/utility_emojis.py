...
class HelpView(discord.ui.View):
    ...
    @discord.ui.button(label="Anterior", style=discord.ButtonStyle.secondary, emoji="<:1000032058:1507947336616251574>")
    async def btn_prev(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        self.page -= 1
        self._update_buttons()
        await interaction.response.edit_message(embed=self.build_embed(), view=self)

    @discord.ui.button(label="Próxima", style=discord.ButtonStyle.secondary, emoji="<:1000032058:1507947336616251574>")
...
class Utility(commands.Cog):
    ...
    async def _ai_complete(self, system: str, user: str, max_tokens: int = 600) -> str:
        if not self.ai:
            return "<:1000032079:1507948213741813972> IA não configurada (OPENAI_API_KEY ausente)."
        try:
    ...
    async def ping(self, interaction: discord.Interaction) -> None:
        ...
        if ws_ms < 100:
            color, status = discord.Color.green(), "<:1000032060:1507947421911613560> Excelente"
        elif ws_ms < 200:
            color, status = discord.Color.gold(), "<:1000032058:1507947336616251574> Normal"
        else:
            color, status = discord.Color.red(), "<:1000032063:1507947553654833232> Alta latência"
    ...
    async def botinfo(self, interaction: discord.Interaction) -> None:
        ...
        embed.set_author(
            name=f"<:1000032075:1507948047269888001> {bot_name}",
            icon_url=bot.user.display_avatar.url if bot.user else None,
        )

        # Descrição de identidade
        embed.description = (
            f"### <a:1000032071:1507947918752092301> Olá! Eu sou o **{bot_name}**\n"
        ...
        embed.add_field(
            name="<:1000032076:1507948144011509820> Servidores",
            value=f"```{guilds:,}```",
            inline=True,
        )
        embed.add_field(
            name="<:1000032081:1507948313549209720> Usuários",
            value=f"```{members:,}```",
            inline=True,
        )
        embed.add_field(
            name="<:1000032049:1507946904124919949> Comandos",
            value=f"```{cmds}```",
            inline=True,
        )
        ...
        embed.add_field(
            name="<:1000032077:1507948183290904736> Latência",
            value=f"```{'<:1000032061:1507947461900111903>' if latency < 100 else '<:1000032079:1507948213741813972>' if latency < 200 else '<a:1000032057:1507947249873719497>'} {latency}ms```",
            inline=True,
        )
        embed.add_field(
            name="<a:1000032071:1507947918752092301> Uptime",
            value=f"```{_uptime_str()}```",
            inline=True,
        )
        embed.add_field(
            name="<:1000032076:1507948144011509820> Canais",
            value=f"```{channels:,}```",
            inline=True,
        )
        ...
        # Tecnologia
        embed.add_field(
            name="<:1000032048:1507946854405505034> Tecnologia",
            value=(
                f"<:1000032055:1507947171624910859> **Python** `{platform.python_version()}`\n"
                f"<:1000032054:1507947088590274580> **discord.py** `{discord.__version__}`\n"
                f"<:1000032072:1507947958723809340> **Sistema** `{platform.system()} {platform.release()}`"
            ),
            inline=True,
        )
        ...
        embed.add_field(
            name="<:1000032072:1507947958723809340> Inteligência Artificial",
            value=(
                f"<:1000032055:1507947171624910859> **OpenAI** IA ativa\n<:1000032068:1507947786367402105> **Modelo** `{AI_MODEL}`"
            ),
            inline=True,
        )
        ...
        embed.add_field(
            name="<:1000032053:1507947052183584910> Desenvolvimento",
            value=(
                "Desenvolvido com <a:1000032071:1507947918752092301> para servidores Discord\n"
    ...
    async def userinfo(self, interaction: discord.Interaction, membro: Optional[discord.Member] = None) -> None:
        ...
        embed = self._base_embed(membro.display_name, color, icon="<:1000032081:1507948313549209720>")
    ...
    async def lembrete(
        ...
        if minutos < 1 or minutos > 10080:
            ...
            return
        await interaction.response.send_message(
            f"<:1000032056:1507947210057322637> Lembrete definido! Vou te avisar em **{minutos} minuto(s)**.", ephemeral=True
        )
