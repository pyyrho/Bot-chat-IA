...
class Moderation(commands.Cog):
    ...
    async def reset_safe_defaults(self, guild_id: int):
        """Garante que configs perigosas estejam desativadas por padrão."""
    ...
    async def safe_delete(self, message: discord.Message) -> bool:
        """Deleta mensagem com tratamento de erros de permissão"""
        try:
    ...
    async def ai_analyze_message(self, content: str) -> dict:
        try:
            response = await asyncio.to_thread(
                ...
                messages=[
                    {"role": "system", "content": """Analise a mensagem e retorne APENAS um JSON:
{"violation": true/false, "severity": "low/medium/high", "reason": "motivo curto em português", "action": "warn/mute/kick/ban/none"}
Só marque violation=true para: ameaças explícitas de violência, hate speech grave, assédio severo, conteúdo sexual explícito.
Conversas normais, discussões, palavrões leves = violation=false. Retorne APENAS o JSON."""},
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.author.guild_permissions.administrator:
            return
        # Verifica se o bot tem permissão de gerenciar mensagens antes de tentar moderar
        ...
        # Moderação por IA — desativada por padrão, só roda se o admin ativar
        if config.get("ai_moderation") and len(content) > 20:
            result = await self.ai_analyze_message(content)
            if result.get("violation"):
                severity = result.get("severity", "low")
                reason = result.get("reason", "Violação detectada")
