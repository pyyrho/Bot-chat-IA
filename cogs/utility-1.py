...
Chave de API: OPENAI_API_KEY (usada para IA geral de utilidades)
"""
...
class HelpView(discord.ui.View):
    ...
    async def btn_close(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.edit_message(
            content="✅ Painel fechado.", embed=None, view=None
        )
        self.stop()
...
class PollView(discord.ui.View):
    def __init__(self, options: list[str], creator_id: int, duration_minutes: int) -> None:
        ...
        end_btn = discord.ui.Button(label="Encerrar", style=discord.ButtonStyle.danger, custom_id="poll_end")
    ...
    async def _end_poll_callback(self, interaction: discord.Interaction) -> None:
        if interaction.user.id != self.creator_id and not interaction.user.guild_permissions.manage_messages:
            await interaction.response.send_message("❌ Apenas o criador da enquete pode encerrá-la.", ephemeral=True)
            return
        self.ended = True
        self.stop()
    ...
    def _build_embed(self, *, final: bool = False) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=("📊 Enquete" if not final else "📊 Enquete Encerrada"),
...
class Utility(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.ai: Optional[AsyncOpenAI] = AsyncOpenAI(api_key=api_key) if api_key else None

    # ── Helpers ───────────────────────────────

    def _base_embed(self, title: str, color: discord.Color, *, icon: str = "✨") -> discord.Embed:
        embed = discord.Embed(
            title=f"{icon} {title}",
            color=color,
            timestamp=datetime.now(timezone.utc),
    ...
    async def _ai_complete(self, system: str, user: str, max_tokens: int = 600) -> str:
        if not self.ai:
            return "⚠️ IA não configurada (OPENAI_API_KEY ausente)."
        try:
    ...
    async def ping(self, interaction: discord.Interaction) -> None:
        ...
        if ws_ms < 100:
            ...
            color, status = discord.Color.gold(), "🟡 Normal"
        else:
            color, status = discord.Color.red(), "🔴 Alta latência"

        embed = self._base_embed("Status dos Serviços", color, icon="📡")
    ...
    async def botinfo(self, interaction: discord.Interaction) -> None:
        ...
        # Cor dinâmica baseada na latência
        if latency < 100:
            color = discord.Color.from_rgb(88, 101, 242)   # blurple vibrante
        elif latency < 200:
            color = discord.Color.from_rgb(250, 166, 26)   # dourado
        else:
            color = discord.Color.from_rgb(237, 66, 69)    # vermelho

        embed = discord.Embed(color=color, timestamp=datetime.now(timezone.utc))

        # Banner do bot (se tiver) ou thumbnail grande
        if bot.user:
            try:
        ...
        # Descrição de identidade
        embed.description = (
            f"### 🤖 Olá! Eu sou o **{bot_name}**\n"
            "Um bot de **IA avançada** feito para transformar a sua experiência no Discord.\n"
            "Trago moderação inteligente, entretenimento e muito mais para o seu servidor!\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )

        # Bloco de estatísticas — linha 1
        embed.add_field(
            name="🏠 Servidores",
            value=f"```{guilds:,}```",
            inline=True,
        ...
        # Bloco de estatísticas — linha 2
        embed.add_field(
            name="📡 Latência",
            value=f"```{'🟢' if latency < 100 else '🟡' if latency < 200 else '🔴'} {latency}ms```",
            inline=True,
        )
        embed.add_field(
            name="⏱️ Uptime",
        ...
        # IA
        openai_ok  = bool(self.ai)
        groq_ok    = bool(os.getenv("GROQ_API_KEY"))
        embed.add_field(
            name="🧠 Inteligência Artificial",
            value=(
                f"{'✅' if openai_ok else '❌'} **OpenAI** (geral)\n"
                f"{'✅' if groq_ok  else '❌'} **Groq** (moderação)\n"
                f"📊 **Modelo** `{AI_MODEL}`"
            ),
            inline=True,
        )
        ...
        # Criado por / links (personalize conforme seu bot)
        embed.add_field(
            name="👨‍💻 Desenvolvimento",
            value=(
                "Desenvolvido com 💜 para servidores Discord\n"
        ...
        # View com botão de suporte (opcional — personalize a URL)
    ...
    async def userinfo(self, interaction: discord.Interaction, membro: Optional[discord.Member] = None) -> None:
        ...
        if flags.active_developer: badges.append("💻 Dev Ativo")
        if membro.premium_since:   badges.append("💎 Server Booster")

        color = membro.color if membro.color != discord.Color.default() else discord.Color.blurple()
        embed = self._base_embed(membro.display_name, color, icon="👤")
    ...
    async def serverinfo(self, interaction: discord.Interaction) -> None:
        ...
        embed = self._base_embed(g.name, discord.Color.blurple(), icon="🏰")
        if g.icon:
        ...
        embed.add_field(
            name="Membros",
            value=f"Total `{g.member_count}` · 👥 `{humans}` · 🤖 `{bots}`",
            inline=False,
        )
        embed.add_field(
            name="Status",
    ...
    async def banner(
        ...
        if not user.banner:
            await interaction.followup.send(
                f"❌ {target.display_name} não possui banner.", ephemeral=True
            )
            return
    ...
    async def roleinfo(self, interaction: discord.Interaction, cargo: discord.Role) -> None:
        ...
        embed = self._base_embed(f"Cargo · {cargo.name}", cargo.color or discord.Color.default(), icon="🏷️")
    ...
    async def canalinfo(
        ...
        embed = self._base_embed(f"Canal · #{ch.name}", discord.Color.blurple(), icon="📝")
    ...
    async def poll(
        ...
        if len(option_list) < 2 or len(option_list) > 5:
            await interaction.response.send_message(
                "❌ Forneça entre 2 e 5 opções separadas por vírgula.", ephemeral=True
            )
            return
    ...
    async def lembrete(
        ...
        if minutos < 1 or minutos > 10080:
            await interaction.response.send_message(
                "❌ Defina entre 1 minuto e 7 dias (10080 minutos).", ephemeral=True
            )
            return
        await interaction.response.send_message(
            f"✅ Lembrete definido! Vou te avisar em **{minutos} minuto(s)**.", ephemeral=True
        )
    ...
    async def timestamp(
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ Data inválida: {exc}", ephemeral=True)
            return
        ...
        embed = self._base_embed("Gerador de Timestamps", discord.Color.blurple(), icon="🕰️")
    ...
    async def cor(self, interaction: discord.Interaction, hex_code: str) -> None:
        ...
        if len(clean) not in (3, 6) or not all(c in "0123456789abcdefABCDEF" for c in clean):
            await interaction.response.send_message("❌ HEX inválido. Use formato `#RRGGBB` ou `#RGB`.", ephemeral=True)
            return
        r, g, b = _hex_to_rgb(clean if len(clean) == 6 else "".join(c * 2 for c in clean))
        h, s, l = _rgb_to_hsl(r, g, b)
        color = discord.Color.from_rgb(r, g, b)
        embed = self._base_embed(f"Cor · #{clean.upper()}", color, icon="🎨")
    ...
    async def resumir(
        ...
        embed = self._base_embed("Resumo (IA)", discord.Color.green(), icon="📄")
    ...
    async def traduzir(
        ...
        embed = self._base_embed(f"Tradução para {idioma_destino.title()}", discord.Color.blurple(), icon="🌐")
    ...
    async def pergunte_ia(self, interaction: discord.Interaction, pergunta: str) -> None:
        ...
        embed = self._base_embed("Resposta da IA", discord.Color.blurple(), icon="💬")
