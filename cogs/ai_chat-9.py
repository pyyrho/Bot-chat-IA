...
async def search_web(query: str, academic: bool = False) -> str:
    try:
        # Para buscas acadêmicas, busca na SEP ou Wikipedia primeiro
        if academic:
            result = await _search_sep(query)
            if result:
                return result
            result = await _search_ddg(query + " site:plato.stanford.edu OR site:philpapers.org OR site:en.wikipedia.org")
            if result:
                return result
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                if resp.status == 200:
                    ...
                    if result:
                        context = f"[Fonte: {result[:600]}"
                        if source_url:
                            context += f" | Link: {source_url}"
                        context += "]"
                        return context
        return ""
    except Exception:
...
async def _search_sep(query: str) -> str:
    """Busca na Stanford Encyclopedia of Philosophy."""
    try:
        # SEP tem uma API de busca simples
        slug = query.lower().strip().replace(" ", "-")
        url = f"https://plato.stanford.edu/entries/{slug}/"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                if resp.status == 200:
                    return f"[Encontrado na Stanford Encyclopedia of Philosophy (SEP): {url}]"
        return ""
    except Exception:
        return ""
...
class AIChat(commands.Cog):
    def __init__(self, bot):
        ...
        if self.gemini_key:
            ...
            logger.info(f"✅ GEMINI_API_KEY carregada ({self.gemini_key[:8]}...) — fallback ativo")
        else:
            self.gemini_model = None
            self.gemini_sessions = {}
            logger.warning("⚠️ GEMINI_API_KEY não encontrada — fallback desativado")
    ...
    async def get_ai_response(self, user_message: str, user_id: int, user_name: str) -> str:
        ...
        try:
            ...
            if is_limit:
                logger.warning(f"Groq com limite — tentando fallback Gemini")
            else:
            ...
            if self.gemini_model:
                try:
                    if user_id not in self.gemini_sessions:
                        self.gemini_sessions[user_id] = self.gemini_model.start_chat(history=[])
                    chat = self.gemini_sessions[user_id]
                    response = await asyncio.to_thread(
                    ...
                    self.last_response_style[user_id] = first_words
                    history.append({"role": "assistant", "content": reply})
                    logger.info("✅ Fallback Gemini usado com sucesso")
                    return reply
                except Exception as e2:
                    logger.error(f"Erro Gemini fallback: {type(e2).__name__}: {e2}")
