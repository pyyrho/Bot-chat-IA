...
class Moderation(commands.Cog):
    ...
    async def ban(
        ...
        deletar_msgs = max(0, min(deletar_msgs, 7))
        motivo = _truncate(motivo, 430)
        await membro.ban(
            reason=f"{motivo} | Por: {interaction.user}",
            delete_message_days=deletar_msgs,
        )
    ...
    async def tempban(
        ...
        if not ok:
            await interaction.response.send_message(msg, ephemeral=True)
            return
        if duracao < 1 or duracao > TEMPBAN_MAX_MINUTES:
            await interaction.response.send_message(
                f"<:1000032056:1507947210057322637> Use uma duração entre 1 e {TEMPBAN_MAX_MINUTES} minutos.",
                ephemeral=True,
            )
            return

        await interaction.response.defer()
        motivo = _truncate(motivo, 450)
        user_id = membro.id
        guild_id = interaction.guild.id
        expires_at = datetime.now(timezone.utc) + timedelta(minutes=duracao)

        await membro.ban(
            reason=f"[TEMPBAN {duracao}min] {motivo} | Por: {interaction.user}",
            delete_message_days=0,
        )
        persisted = await self._store_tempban(guild_id, user_id, expires_at, motivo)
        if not persisted:
            asyncio.create_task(self._fallback_unban_later(guild_id, user_id, duracao * 60))
    ...
    async def mute(
        ...
        if not ok:
            ...
            return
        await interaction.response.defer()
        duracao = max(1, min(duracao, 40320))
        success = await self.mute_member(
            interaction.guild, membro, duracao * 60, motivo, moderator=interaction.user
        )
        if success:
            embed = self._base_embed("Usuário Silenciado", discord.Color.orange(), icon="<:1000032059:1507947381096714260>")
