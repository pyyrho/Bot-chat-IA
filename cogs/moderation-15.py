...
class Moderation(commands.Cog):
    ...
    def _base_embed(
        ...
        *,
        ...
        embed = discord.Embed(
            title=f"{icon} {title}".strip() if icon else title,
            color=WHITE,
    ...
    async def _handle_mention_action(self, message: discord.Message) -> bool:
        ...
        ban_triggers = ("bane ", "bane\n", "banir ", "bana ", "ban o ", "ban a ")
        ...
        # unmute deve ser checado ANTES de mute para evitar que "desmuta"
        # seja capturado como "muta" (substring match)
        unmute_triggers = ("desmuta ", "desmuta\n", "desilencia ", "unmute ", "remove silêncio", "remove silencio", "desmutar ", "dessilencia ")
        mute_triggers = ("muta o ", "muta a ", "muta\n", "silencia o ", "silencia a ", "silenciar ", "mute o ", "mute a ", "timeout ")
        warn_triggers = ("avisa ", "aviso para ", "avisar ", "warn ")

        # Verificação de unmute SEMPRE antes de mute
        ...
        is_warn = (not is_mute and not is_unmute) and any(t in low for t in warn_triggers)

        if not any((is_ban, is_tempban, is_softban, is_kick, is_mute, is_unmute, is_warn)):
            return False

        # Marca o gate imediatamente após detectar a intenção, antes de qualquer
        # chamada de API. Isso garante que o ai_chat.py (que espera 0.35s) veja
        # o gate marcado antes de decidir responder.
        ...
        if not target:
            await message.reply(
                "Mencione o usuário que deseja moderar. "
        ...
        if (is_ban or is_tempban or is_softban) and not perms.ban_members:
            await message.reply("Você não tem permissão para banir membros.", mention_author=False)
            return True
        if is_kick and not perms.kick_members:
            await message.reply("Você não tem permissão para expulsar membros.", mention_author=False)
            return True
        if (is_mute or is_unmute) and not perms.moderate_members:
            await message.reply("Você não tem permissão para silenciar membros.", mention_author=False)
            return True
        if is_warn and not perms.manage_messages:
            await message.reply("Você não tem permissão para registrar avisos.", mention_author=False)
            return True
        ...
        try:
            if is_ban:
                ...
                embed = self._base_embed("Ban Temporário", discord.Color.red(), icon="")
                embed.add_field(name="Usuário", value=f"{target} (`{target.id}`)", inline=True)
                ...
                await message.reply(
                    f"{target.mention} foi desmutado.",
                    mention_author=False,
                )
                ...
                if not success:
                    await message.reply(
                        f"Não consegui silenciar {target.mention}.",
                        mention_author=False,
                    )
                else:
                    await message.reply(
                        f"{target.mention} foi silenciado por {minutes} minuto(s).",
                        mention_author=False,
                    )
                ...
                await message.reply(
                    f"Aviso registrado para {target.mention}.",
                    mention_author=False,
                )

        except discord.Forbidden:
            await message.reply(
                "Não tenho permissão para realizar essa ação. "
                "Verifique se meu cargo está acima do alvo.",
                mention_author=False,
            )
        except discord.HTTPException as exc:
            await message.reply(f"Erro ao moderar: {exc}", mention_author=False)
