...
Arquitetura híbrida:
  ...
  • Todo o resto funciona por MENÇÃO, estilo Jarvis:
      ...
      "@RevolutX não responda mais @Usuário"  (somente administrador)
      "@RevolutX desbloqueie @Usuário"        (somente administrador)
...
Variáveis de ambiente:
  GEMINI_API_KEY, GEMINI_API_KEY_2... — chaves usadas pelos comandos de IA
  UTIL_AI_MODEL    — modelo das utilidades (padrão: gemini-3.6-flash)
...
UTILITY_BUILD = "2026-08-21-ajuda-academico-v3-fonte-primeiro-gemini-3.6"
...
def _discover_gemini_api_keys() -> list[tuple[str, str]]:
    """Descobre todas as GEMINI_API_KEY numeradas e remove duplicatas."""
...
AI_MODEL = os.getenv("UTIL_AI_MODEL", "gemini-3.6-flash")
...
_HELP_PAGES: list[tuple[str, Callable[[], discord.Color], str]] = [
    (
        ...
        (
            ...
            "**Modo Acadêmico V3 por menção:**\n"
            "`@Revolux acadêmico iniciar [tema]` · Abre uma sala isolada\n"
            "`@Revolux acadêmico status` · Mostra objetivo, nível e método\n"
            "`@Revolux acadêmico exportar` · Baixa as notas em Markdown\n"
            "`@Revolux acadêmico encerrar` · Finaliza a sessão\n"
            "Aceita imagens, PDF e arquivos de texto. O painel permite escolher "
            "Aprender, Resolver, Verificar, Artigo ou Argumento. Perguntas acadêmicas "
            "usam fontes recuperadas e mostram referências `[S#]`; se a busca obrigatória "
            "falhar, o bot não completa os fatos no chute.\n\n"
    ...
    (
        ...
        (
            ...
            "**Controle de acesso à IA — somente administradores:**\n"
            "`@Revolux não responda mais @usuário` · Bloquear respostas neste servidor\n"
            "`@Revolux desbloqueie @usuário` · Liberar o usuário novamente\n"
            "Também funciona informando o ID ou respondendo à mensagem do usuário.\n\n"
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if guild_id and await ai_response_blocklist.is_blocked(
            ...
            await interaction.response.send_message(
                "Seu acesso ao Revolux está bloqueado neste servidor.",
                ephemeral=True,
            )
            return False
        return True
...
def _can_end_poll(user_id: int, creator_id: int) -> bool:
    """Somente quem criou a enquete pode encerrá-la manualmente."""
...
class PollView(LayoutView):
    ...
    async def _end_poll(self, interaction: discord.Interaction) -> None:
        if not _can_end_poll(interaction.user.id, self.creator_id):
            await interaction.response.send_message(
                f"{EMOJI_ERROR} Apenas quem criou a enquete pode encerrá-la.",
                ephemeral=True,
            )
            return
...
class Utility(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        logger.info("Versão ativa do Utility: %s", UTILITY_BUILD)

        for env_name, api_key in _discover_gemini_api_keys():
            try:
        ...
        async with self._ai_lock:
            ...
            for offset in range(total_clients):
                ...
                try:
                    ...
                    if not AI_MODEL.lower().strip().startswith("gemini-3"):
    ...
    async def _deny_blocked_interaction(
        ...
        """Bloqueia slash commands de forma privada no servidor da restrição."""
        guild_id = interaction.guild_id
        if not guild_id:
            return False
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        # Bloqueados não recebem respostas das utilidades por menção. O
        # ai_chat.py aplica a mesma regra a menções, respostas e canais de IA.
    ...
    def _detect_intent(self, message: discord.Message, low: str, content: str):
        ...
        checks: list[tuple[bool, Callable]] = [
            (self._matches_unblock_user(low), self._intent_unblock_user),
            (self._matches_block_user(low), self._intent_block_user),
    ...
    @staticmethod
    def _matches_block_user(low: str) -> bool:
        value = low.strip()
        direct_phrase = re.match(
            r"^(?:n[aã]o respond(?:a|e) mais|pare de responder)\b",
            value,
        )
        explicit_target = re.match(
            r"^(?:bloque(?:ie|ar)|ignore|ignorar)\s+"
            r"(?:<@!?\d{17,20}>|\d{17,20}\b|"
            r"(?:(?:o|a|esse|essa)\s+)?(?:usu[aá]rio|membro|rapaz|pessoa)\b)",
            value,
        )
        return bool(direct_phrase or explicit_target)

    @staticmethod
    def _matches_unblock_user(low: str) -> bool:
        value = low.strip()
        direct_phrase = re.match(
            r"^(?:volte a responder|pode responder novamente)\b",
            value,
        )
        explicit_target = re.match(
            r"^desbloque(?:ie|ar)\s+"
            r"(?:<@!?\d{17,20}>|\d{17,20}\b|ele\b|ela\b|"
            r"(?:(?:o|a|esse|essa)\s+)?(?:usu[aá]rio|membro|rapaz|pessoa)\b)",
            value,
        )
        return bool(direct_phrase or explicit_target)
    ...
    def _matches_userinfo(low: str) -> bool:
        # "Quem é X?" é uma pergunta normal para a IA. A utilidade só assume o
        # pedido quando o usuário nomeia explicitamente perfil/info de membro.
        return bool(
            re.match(
                r"^(?:userinfo|info do usu[aá]rio|informa[cç][oõ]es do usu[aá]rio|"
                r"perfil do usu[aá]rio|dados do usu[aá]rio)\b",
                low,
            )
        )
    ...
    def _matches_avatar(low: str) -> bool:
        command = r"(?:avatar|ver avatar|mostrar avatar|mostra(?:r)? (?:o )?avatar)"
        return bool(
            re.fullmatch(command, low.strip())
            or re.fullmatch(
                command
                + r"\s+(?:(?:de|do|da)\s+.+|<@!?\d{17,20}>|@\S+|\d{17,20})",
                low.strip(),
            )
        )
    ...
    def _matches_banner(low: str) -> bool:
        command = r"(?:banner|ver banner|mostrar banner|mostra(?:r)? (?:o )?banner)"
        return bool(
            re.fullmatch(command, low.strip())
            or re.fullmatch(
                command
    ...
    def _matches_serverinfo(low: str) -> bool:
        return bool(
            re.match(
                r"^(?:info do servidor|serverinfo|servidor info|"
                r"informa[cç][oõ]es do servidor)\b",
                low,
            )
        )
    ...
    def _matches_roleinfo(low: str) -> bool:
        return bool(
            re.match(
                r"^(?:info do cargo|roleinfo|cargo info|informa[cç][oõ]es do cargo)\b",
                low,
            )
        )
    ...
    def _matches_canalinfo(low: str) -> bool:
        return bool(
            re.match(
                r"^(?:info do canal|canalinfo|canal info|informa[cç][oõ]es do canal)\b",
                low,
            )
        )
    ...
    def _matches_poll(low: str) -> bool:
        return bool(
            re.match(r"^(?:enquete|poll)\b", low)
            or re.match(
                r"^(?:crie|criar|fa[cç]a|nova)\s+(?:uma\s+)?"
                r"(?:enquete|poll|vota[cç][aã]o)\b",
                low,
            )
        )
    ...
    def _matches_lembrete(low: str) -> bool:
        starts_as_command = bool(
            re.match(
                r"^(?:me lembra|lembre-me|lembre me|crie (?:um )?lembrete|"
                r"lembrete|lembrar em)\b",
                low,
            )
        )
        has_duration = bool(
            re.search(r"(?:em|daqui)\s+\d+\s*(?:minutos?|min|m\b)", low)
        )
        return starts_as_command and has_duration

    @staticmethod
    def _matches_timestamp(low: str) -> bool:
        return bool(re.match(r"^(?:timestamp|gerar timestamp)\b", low))

    @staticmethod
    def _matches_cor(low: str, content: str) -> bool:
        # Exige comando no início E um HEX com '#'. Antes, "concorda ... 2004"
        # virava "cor #000044" porque "cor" aparecia dentro de "concorda" e
        # três dígitos do ano eram aceitos como HEX.
        command = re.match(r"^(?:cor|color|hex)\b", low)
        valid_hex = re.search(
            r"(?<![0-9A-Za-z])#(?:[0-9a-fA-F]{6}|[0-9a-fA-F]{3})\b",
            content,
        )
        return bool(command and valid_hex)

    @staticmethod
    def _matches_resumir(low: str) -> bool:
        # A utilidade resume texto COLADO após ':'. Pedidos como "me resuma as
        # ideias de X" pertencem à IA conversacional e não entram aqui.
        return bool(
            re.match(
                r"^(?:resume|resuma|resumir|resumo(?:\s+de)?)"
                r"(?:\s+(?:este|esse|o seguinte)\s+texto)?\s*:\s*\S",
                low,
                re.DOTALL,
            )
        )
    ...
    def _matches_traduzir(low: str) -> bool:
        return bool(
            re.match(
                r"^(?:traduz|traduza|traduzir|translate)\b[^:\n]{0,80}:\s*\S",
                low,
                re.DOTALL,
            )
        )
    ...
    async def _resolve_block_target(
        ...
        """Resolve o alvo por menção, ID ou autor da mensagem respondida."""
        guild = message.guild
        if not guild:
            return None

        # Para ações administrativas não há busca aproximada por nome: o alvo
        # deve ser inequívoco e sempre acaba identificado pelo ID do Discord.
        for mentioned in message.mentions:
            if self.bot.user and mentioned.id == self.bot.user.id:
                continue
            member = guild.get_member(mentioned.id)
            if member is not None:
                return member

        raw_id = re.search(r"\b(\d{17,20})\b", text)
        if raw_id:
            member = guild.get_member(int(raw_id.group(1)))
            if member is not None:
                return member
            try:
                return await guild.fetch_member(int(raw_id.group(1)))
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
                pass
        ...
        if author is not None:
            cached = guild.get_member(getattr(author, "id", 0))
            if cached is not None:
                return cached
        return None

    @staticmethod
    def _author_is_admin(message: discord.Message) -> bool:
    ...
    async def _intent_block_user(
        self, message: discord.Message, content: str, low: str
    ) -> bool:
        if not self._matches_block_user(low):
            return False
        if not self._author_is_admin(message):
            await self._send_error(
                message, "Somente um administrador pode bloquear respostas para alguém."
            )
            return True

        guild = message.guild
        target = await self._resolve_block_target(message, content)
        if not guild or target is None:
            await self._send_error(
                message,
                "Mencione o usuário, informe o ID dele ou responda à mensagem dele.",
            )
            return True
        if target.id == message.author.id:
            await self._send_error(message, "Você não pode bloquear a si mesmo.")
            return True
        if self.bot.user and target.id == self.bot.user.id:
            await self._send_error(message, "Não posso bloquear a mim mesmo.")
            return True
        if target.bot:
            await self._send_error(message, "Esse bloqueio aceita apenas usuários.")
            return True

        try:
            created = await ai_response_blocklist.block(
                guild.id, target.id, message.author.id
            )
        except BlocklistUnavailable:
            logger.exception("Falha ao salvar bloqueio de respostas")
            await self._send_error(
                message,
                "Não consegui salvar o bloqueio no banco de dados. Tente novamente.",
            )
            return True

        if created:
            text = (
                f"Vou ignorar {target.mention} neste servidor até que um "
                "administrador o desbloqueie."
            )
        else:
            text = f"{target.mention} já estava bloqueado neste servidor."
        await self._send_confirmation(message, text)
        return True

    async def _intent_unblock_user(
        self, message: discord.Message, content: str, low: str
    ) -> bool:
        if not self._matches_unblock_user(low):
            return False
        if not self._author_is_admin(message):
            await self._send_error(
                message, "Somente um administrador pode desbloquear alguém."
            )
            return True
        ...
        try:
            removed = await ai_response_blocklist.unblock(guild.id, target.id)
        except BlocklistUnavailable:
            logger.exception("Falha ao remover bloqueio de respostas")
            await self._send_error(
                message,
                "Não consegui remover o bloqueio no banco de dados. Tente novamente.",
            )
            return True

        if removed:
            text = f"{target.mention} foi desbloqueado e pode usar o bot novamente."
        else:
            text = f"{target.mention} não estava bloqueado neste servidor."
        await self._send_confirmation(message, text)
        return True
    ...
    async def _intent_cor(
        ...
        if not self._matches_cor(low, content):
            return False
    ...
    async def _intent_resumir(
        ...
        if not self._matches_resumir(low):
            return False

        text_match = re.match(
            r"^(?:resume|resuma|resumir|resumo(?:\s+de)?)"
            r"(?:\s+(?:este|esse|o seguinte)\s+texto)?\s*:\s*(.+)$",
            content,
    ...
    async def _intent_traduzir(
        ...
        if not self._matches_traduzir(low):
            return False
