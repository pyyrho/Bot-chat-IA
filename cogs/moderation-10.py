...
Chave de API: GROQ_API_KEY ou MOD_GROQ_API_KEYS (somente para moderação por IA)
"""
...
AI_MODEL         = os.getenv("MOD_GROQ_MODEL", os.getenv("MOD_AI_MODEL", "llama-3.3-70b-versatile"))
AI_DEEP_MODEL    = os.getenv("MOD_GROQ_DEEP_MODEL", os.getenv("MOD_AI_DEEP_MODEL", "llama-3.3-70b-versatile"))   # análise profunda via Groq
...
# ──────────────────────────────────────────────
# Cliente Groq dedicado à moderação
# ──────────────────────────────────────────────

def _collect_groq_keys() -> list[str]:
    """
    Lê uma ou várias chaves Groq sem usar a chave da OpenAI.
    ...
    """
    keys: list[str] = []
    for env_name in ("MOD_GROQ_API_KEYS", "GROQ_API_KEYS"):
        raw = os.getenv(env_name, "")
        keys.extend(k.strip() for k in raw.replace(";", ",").split(",") if k.strip())

    for env_name in ("MOD_GROQ_API_KEY", "GROQ_API_KEY"):
...
class Moderation(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        # Moderação por IA usa Groq/GroqCloud, não OpenAI.
        # A IA geral do bot pode continuar usando OPENAI_API_KEY em outros cogs.
        groq_keys = _collect_groq_keys()
        self._groq_index = 0
        self._groq_clients: list[AsyncOpenAI] = [
            AsyncOpenAI(
                api_key=key,
        ...
        self.ai: Optional[AsyncOpenAI] = self._groq_clients[0] if self._groq_clients else None
        if not self.ai:
            logger.warning("Moderação por IA desativada: GROQ_API_KEY/MOD_GROQ_API_KEYS ausente.")
    ...
    async def _groq_chat(self, *, model: str, messages: list[dict], **kwargs):
        """
        Executa chat completion no Groq com failover entre múltiplas chaves.
        Não chama OpenAI. Se uma chave falhar por limite/erro temporário,
        tenta a próxima até esgotar a lista.
        """
        if not self._groq_clients:
            raise RuntimeError("Groq não configurado")

        attempts = len(self._groq_clients)
        last_exc: Optional[Exception] = None

        for _ in range(attempts):
            client = self._groq_clients[self._groq_index % attempts]
            self._groq_index = (self._groq_index + 1) % attempts
            try:
                ...
                last_exc = exc
                logger.warning("Falha em chamada Groq; tentando próxima chave se existir: %s", exc)
                await asyncio.sleep(0.35)

        raise last_exc or RuntimeError("Falha desconhecida no Groq")
