...
Chave de API: GROQ_API_KEY (exclusiva para moderação, separada da OpenAI)
"""
...
AI_MODEL         = os.getenv("MOD_AI_MODEL", "llama-3.3-70b-versatile")
AI_DEEP_MODEL    = os.getenv("MOD_AI_DEEP_MODEL", "llama-3.1-70b-versatile")   # análise profunda
...
class Moderation(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.ai: Optional[AsyncGroq] = AsyncGroq(api_key=api_key) if api_key else None
    ...
    def can_act_on(
        ...
        if actor != guild.owner and target.top_role >= actor.top_role:
            return False, "❌ Seu cargo não é alto o suficiente."
        me = guild.me
        if me and target.top_role >= me.top_role:
            return False, "❌ Meu cargo não é alto o suficiente."
        return True, ""
    ...
    async def mute_member(
        ...
        try:
            ...
            embed = self._base_embed("Usuário Silenciado", discord.Color.orange(), icon="🔇")
    ...
    async def warn_user(
        ...
        embed = self._base_embed("Aviso Registrado", discord.Color.yellow(), icon="⚠️")
    ...
    async def _ai_analyze(
        ...
        *,
        ...
        """
        Envia mensagem + contexto para análise de IA via Groq.
    ...
    async def _ai_summarize_user(
        ...
        if not self.ai or not warnings:
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if config.get("ai_moderation") and len(content.strip()) >= 15:
            ...
            if result.get("violation") and result.get("confidence", 0) >= 0.60:
                ...
                # Log de IA detalhado
                ai_embed = self._base_embed("IA · Violação Detectada", discord.Color.dark_red(), icon="🤖")
                ...
                if result.get("slow_mode_suggestion") and isinstance(message.channel, discord.TextChannel):
                    ...
                    if not self._slowmode_active.get(key):
                        try:
                            ...
                            sm_embed = self._base_embed("Slow-mode Ativado pela IA", discord.Color.gold(), icon="🐢")
    ...
    async def _remove_slowmode_after(
        ...
        try:
            ...
            embed = self._base_embed("Slow-mode Removido", discord.Color.green(), icon="✅")
    ...
    async def ban(
        ...
        embed = self._base_embed("Usuário Banido", discord.Color.red(), icon="🔨")
    ...
    async def tempban(
        ...
        embed = self._base_embed("Ban Temporário", discord.Color.red(), icon="⏱️")
        embed.add_field(name="Usuário", value=f"{membro} (`{membro.id}`)", inline=True)
        ...
        async def _unban_later() -> None:
            ...
            if guild:
                try:
                    ...
                    unban_embed = self._base_embed("Tempban Expirado", discord.Color.green(), icon="✅")
    ...
    async def softban(
        ...
        embed = self._base_embed("Softban Aplicado", discord.Color.orange(), icon="🧹")
        embed.add_field(name="Usuário", value=f"{membro} (`{membro.id}`)", inline=True)
        embed.add_field(name="Moderador", value=interaction.user.mention, inline=True)
        embed.add_field(name="Motivo", value=motivo, inline=False)
    ...
    async def kick(
        ...
        embed = self._base_embed("Usuário Expulso", discord.Color.orange(), icon="👢")
        embed.set_thumbnail(url=membro.display_avatar.url)
        embed.add_field(name="Usuário", value=f"{membro} (`{membro.id}`)", inline=True)
        embed.add_field(name="Moderador", value=interaction.user.mention, inline=True)
        embed.add_field(name="Motivo", value=motivo, inline=False)
    ...
    async def mute(
        ...
        if success:
            ...
            await interaction.followup.send("❌ Não consegui silenciar esse usuário.")
    ...
    async def unmute(self, interaction: discord.Interaction, membro: discord.Member) -> None:
        ...
        try:
            ...
            await interaction.followup.send("❌ Sem permissão para desmutar.")
    ...
    async def warn(
        ...
        await interaction.followup.send(f"✅ Aviso registrado para {membro.mention}.", ephemeral=True)
    ...
    async def remover_aviso(
        ...
        if removed:
            await interaction.response.send_message(f"✅ Aviso `#{id_aviso}` de {membro.mention} removido.", ephemeral=True)
        else:
            await interaction.response.send_message("❌ Aviso não encontrado neste servidor.", ephemeral=True)
    ...
    async def avisos(self, interaction: discord.Interaction, membro: discord.Member) -> None:
        ...
        if not warnings:
            await interaction.followup.send(f"✅ {membro.mention} não possui avisos.", ephemeral=True)
            return
        embed = self._base_embed(f"Avisos · {membro.display_name}", discord.Color.yellow(), icon="⚠️")
        embed.set_thumbnail(url=membro.display_avatar.url)
        lines: list[str] = []
    ...
    async def limpar_avisos(self, interaction: discord.Interaction, membro: discord.Member) -> None:
        ...
        await interaction.response.send_message(
            f"✅ Todos os avisos de {membro.mention} foram removidos e o risco foi zerado.",
            ephemeral=True,
        )
    ...
    async def inspecionar(self, interaction: discord.Interaction, membro: discord.Member) -> None:
        ...
        embed = self._base_embed(f"Inspeção · {membro.display_name}", discord.Color.blurple(), icon="🔍")
        embed.set_thumbnail(url=membro.display_avatar.url)
        embed.add_field(name="Usuário", value=f"{membro} (`{membro.id}`)", inline=True)
        ...
        embed.add_field(name="📊 Análise IA", value=summary, inline=False)
    ...
    async def nota(
        ...
        await interaction.response.send_message(
            f"📝 Nota adicionada ao perfil de {membro.mention}.", ephemeral=True
        )
    ...
    async def purge(
        ...
        embed = self._base_embed("Purge Executado", discord.Color.blurple(), icon="🗑️")
    ...
    async def lockdown(
        ...
        embed = self._base_embed(action_text, color, icon="🔒" if ativar else "🔓")
    ...
    async def palavra_proibida(
        ...
        if acao in {"add", "adicionar"}:
            ...
            if not normalized:
                await interaction.response.send_message("❌ Informe uma palavra.", ephemeral=True)
                return
            ...
            await interaction.response.send_message(f"✅ Palavra adicionada: `{palavra}`", ephemeral=True)

        elif acao in {"remove", "remover"}:
            normalized = palavra.strip().lower()
            words = [w for w in words if w.lower() != normalized]
            await self.save_config(interaction.guild.id, banned_words=words)
            await interaction.response.send_message(f"✅ Palavra removida: `{palavra}`", ephemeral=True)
    ...
    async def config_mod(
        ...
        if warn_action is not None:
            if warn_action.lower() not in {"mute", "kick", "ban"}:
                await interaction.response.send_message(
                    "❌ `warn_action` deve ser `mute`, `kick` ou `ban`.", ephemeral=True
                )
                return
        ...
        config = await self.get_config(interaction.guild.id)
        embed = self._base_embed("Configuração de Moderação", discord.Color.blurple(), icon="⚙️")
